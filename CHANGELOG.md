...
- [UI / Bug fix / Feature] agy sessions now mirror tool calls and sub-agents into the web UI, and no longer (#3890)
...
- [Bug fix / Feature / Chore] Switching between conversations is instant — background conversations stay (#4113)
...
- [Bug fix] Duplicate-detection comments no longer ask you to close your issue when the (#4313)
- [Bug fix] Kimi and Hermes sessions launch again — their version checks compared against (#4314)
- [UI / Feature] Organizations can preconfigure Omnigent server URLs through Android managed (#4315)
- [Feature] `omnigent host --background` starts the local server and registers this machine (#4317)
...
- [Feature] `omnigent start` starts the local server and registers this machine as a host — (#4321)
...
- [Bug fix] Resuming a claude-native session no longer drops a message sent right after the (#4403)
...
- * Fixes an HTTP client resource leak when OpenAI Agents SDK executors shut down. (#4508)
- * Honor OmnigentClient timeout for ordinary Python SDK HTTP requests. (#4509)
...
- [Bug fix] Claude native sessions now report per-turn token usage (`gen_ai.usage.input_tokens` / (#4530)
...
- [UI / Bug fix] The desktop server picker moved from the window title bar to the bottom of the (#4551)
...
- [Bug fix] The Android app no longer shows the Databricks workspace navigation bar around (#4555)
- [UI / Feature] On the macOS desktop app, the sidebar header now shares the title-bar row with (#4557)
...
- [UI] Connecting the iOS app to a Databricks workspace now opens Omnigent directly and (#4559)
...
- [UI / Bug fix] Fixed iOS controls rendering under the status bar on Databricks workspace-hosted (#4568)
...
- [UI / Bug fix] The workspace Files panel now shows hidden files by default, and its eye icon (#4575)
...
- [Chore] Removed the "A new version of Omnigent is available" prompt and browser PWA (#4617)
...
- [Docs] N/A — internal comments and a setup hint. (#4925)
- [Bug fix] `omni setup` and the New Chat picker no longer show a duplicate — or silently (#4927)
