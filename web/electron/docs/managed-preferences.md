...
server as a host. On any other server the flag reads as disabled. Selecting it asks for native confirmation, then runs
`arca ssh` with a remote `isaac omni host --background --non-interactive`
command; the instance authenticates with its own Databricks credentials. After
a successful connect the app also maintains a background `arca ssh` keepalive
session for as long as it runs, so the Arca instance is not suspended for
idleness while serving as a host. Like the server list, the flag is read from
