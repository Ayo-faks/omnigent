...
describe("Run on Arca (Databricks-internal, MDM-gated)", () => {
  ...
  it("offers the Arca row only when the desktop shell reports the MDM flag", async () => {
    renderLanding();
    await openHostMenu();
    expect(await screen.findByTestId("new-chat-landing-run-on-arca")).toBeTruthy();
  });

  it("hides the Arca row when the flag is off or unknown (old shell)", async () => {
    vi.mocked(getDesktopFeatures).mockResolvedValue(null);
    renderLanding();
    await openHostMenu();
    // The menu is open (the escape hatch renders), but no Arca row.
    await screen.findByTestId("new-chat-landing-connect-host");
    expect(screen.queryByTestId("new-chat-landing-run-on-arca")).toBeNull();
  });

  it("surfaces a connect failure with a retry", async () => {
